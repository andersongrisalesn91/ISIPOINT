...
public class CustomTapPolygon implements KmlFeature.Styler {
     ...
     */
    private final transient EventosMapaVendedor eventosMapaVendedor;
     ...
     */
    private final Map<String, InfoWindowCeldaOcupada> celdasEstado = new HashMap<>();

    /**
     * El constructor.
     * @param eventosMapaVendedor el objeto que procesa los eventos
     ...
     * @param celdaRepository el manejador de celdas
     * @param defaultStyle el estilo por defecto
     */
    public CustomTapPolygon(final EventosMapaVendedor eventosMapaVendedor, final KmlDocument kmlDocument,
                            final MapView mapView, final CeldaRepository celdaRepository,
        ...
        this.configMapa = eventosMapaVendedor.getConfigService().getConfig().getMapa();
    }
    ...
    public CustomTapPolygon(final EventosMapaVendedor eventosMapaVendedor, final KmlDocument kmlDocument,
                            final MapView mapView, final CeldaRepository celdaRepository) {
        this(eventosMapaVendedor, kmlDocument, mapView, celdaRepository, null);
    }
    /**
     ...
     */
    private InfoWindow getInfoWindow(final KmlPlacemark kmlPlacemark, final Polygon polygon) {
        final InfoWindow info;
        if (esZona(kmlPlacemark.mId)) {
            info = new InfoWindowZona(mapView, eventosMapaVendedor);
            ...
            if (idTipoCelda == InfoWindowCeldaMultiple.ID_TIPO_CELDA_MULTIPLE) {
                ...
                info = new InfoWindowCeldaMultiple(mapView, eventosMapaVendedor);
            } else {
                InfoWindowCeldaOcupada infoCelda = new InfoWindowCeldaOcupada(mapView,
                        eventosMapaVendedor, kmlPlacemark, polygon);
                // Actualiza el estado
                Integer idEstadoCelda = celdaRepository.getIdEstadoCelda(infoCelda.getIdZona(), infoCelda.getIdCelda());
                // AppLog.debug(TAG, "InfoWindow zona %s celda %s estado %s", infoCelda.getIdZona(), infoCelda.getIdCelda(), idEstadoCelda);
                if (idEstadoCelda != null) {
                    infoCelda.setIdEstado(idEstadoCelda);
                }
                info = infoCelda;
                celdasEstado.put(getKey(infoCelda.getIdZona(), infoCelda.getIdCelda()), infoCelda);
            }
        } else {
            info = null;
        }
    ...
    public final void onPoint(final Marker marker, final KmlPlacemark kmlPlacemark, final KmlPoint kmlPoint) {
        ...
        marker.setInfoWindow(getInfoWindow(kmlPlacemark, null));
    }
    ...
    public final void onPolygon(final Polygon polygon, final KmlPlacemark kmlPlacemark, final KmlPolygon kmlPolygon) {
        ...
        polygon.setInfoWindow(getInfoWindow(kmlPlacemark, polygon));
    }
     ...
     */
    public final void actualizarEstadoCelda(final int idZona, final int idCelda, final int idEstado) {
        final InfoWindowCeldaOcupada infoCelda = celdasEstado.get(getKey(idZona, idCelda));
        if (infoCelda != null) {
            infoCelda.setIdEstado(idEstado);
        }
        AppLog.debug(TAG, "MQTT Actualizando estado idZona %s idCelda %s idEstado %s infoCelda %s",
                idZona, idCelda, idEstado, infoCelda);
    }
